...
public class InvTweaksGuiSettingsAdvanced extends InvTweaksGuiSettingsAbstract {
	...
	private final static int ID_SETTINGS_BUTTON = 6;
	...
	private static String labelSettingsButton;
	...
	public InvTweaksGuiSettingsAdvanced(Minecraft mc_, GuiScreen parentScreen_, InvTweaksConfig config_) {
		...
		labelSettingsButton = I18n.format("invtweaks.settings.settingsbutton");
	}
	...
	public void initGui() {
		...
		InvTweaksGuiTooltipButton sortOnPickupBtn = new InvTweaksGuiTooltipButton(ID_SORT_ON_PICKUP, p.getX(), p.getY(),
				...
				I18n.format("invtweaks.settings.advanced.sortonpickup.tooltip"));
		...
		InvTweaksGuiTooltipButton enableSoundsBtn = new InvTweaksGuiTooltipButton(ID_ENABLE_SOUNDS, p.getX(), p.getY(),
				...
				I18n.format("invtweaks.settings.advanced.sounds.tooltip"));
		...
		controlList.add(new InvTweaksGuiTooltipButton(ID_CHESTS_BUTTONS, p.getX(), p.getY(),
				...
				I18n.format("invtweaks.settings.chestbuttons.tooltip")));

		moveToButtonCoords(i++, p);
		@NotNull
		InvTweaksGuiTooltipButton autoEquipArmorBtn = new InvTweaksGuiTooltipButton(ID_AUTO_EQUIP_ARMOR, p.getX(),
				p.getY(), computeBooleanButtonLabel(InvTweaksConfig.PROP_ENABLE_AUTO_EQUIP_ARMOR, labelEquipArmor),
				I18n.format("invtweaks.settings.advanced.autoequip.tooltip"));
		...
		InvTweaksGuiTooltipButton serverAssistBtn = new InvTweaksGuiTooltipButton(ID_SERVER_ASSIST, p.getX(), p.getY(),
				...
				I18n.format("invtweaks.settings.advanced.serverassist.tooltip"));
		controlList.add(serverAssistBtn);

		moveToButtonCoords(i++, p);
		controlList.add(new InvTweaksGuiTooltipButton(ID_SETTINGS_BUTTON, p.getX(), p.getY(),
				computeBooleanButtonLabel(InvTweaksConfig.PROP_SHOW_SETTINGS_BUTTON, labelSettingsButton),
				I18n.format("invtweaks.settings.settingsbutton.tooltip")));

		moveToButtonCoords(i++, p);
		@NotNull
		InvTweaksGuiTooltipButton displayTooltipBtn = new InvTweaksGuiTooltipButton(ID_DISPLAY_TOOLTIP, p.getX(),
				p.getY(), computeBooleanButtonLabel(InvTweaksConfig.PROP_TOOLTIP_PATH, labelDisplayTooltip),
				I18n.format("invtweaks.settings.displaytooltip.tooltip"));
	...
	protected void actionPerformed(@NotNull GuiButton guibutton) {
		...
		case ID_SERVER_ASSIST:
			...
			InvTweaksMod.proxy.setServerAssistEnabled(!InvTweaks.getConfigManager().getConfig()
					.getProperty(InvTweaksConfig.PROP_ENABLE_SERVER_ITEMSWAP).equals(InvTweaksConfig.VALUE_FALSE));
			break;

		// Toggle settings button
		case ID_SETTINGS_BUTTON:
			toggleBooleanButton(guibutton, InvTweaksConfig.PROP_SHOW_SETTINGS_BUTTON, labelSettingsButton);
			break;
