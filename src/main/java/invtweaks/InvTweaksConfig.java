...
public class InvTweaksConfig {
	...
	public static final String PROP_SHOW_SETTINGS_BUTTON = "showSettingsButton";
	...
	/**
	 * Returns the file when the properties are stored, after making sure the file
	 * exists.
	 *
	...
	public void load() throws Exception {
		...
		synchronized (this) {
			...
			if (currentRulesetName != null) {
				int rulesetIndex = 0;
				for (@NotNull
				InvTweaksConfigInventoryRuleset ruleset : rulesets) {
	...
	/**
	 * @return The inventory slots array indicating which ones are frozen. WARNING:
	 *         Not a copy.
	 */
	...
	public boolean isAutoRefillEnabled(String itemID, int itemDamage) {
		if (!getProperty(PROP_ENABLE_AUTO_REFILL).equals(VALUE_FALSE)) {
			...
			boolean found = false;
			for (@NotNull
			String keyword : autoReplaceRules) {
	...
	private void reset() {
		...
		properties.put(PROP_SHOW_SETTINGS_BUTTON, VALUE_TRUE);
	...
	private void loadProperties() throws IOException {
		...
		if (newProperties.get(PROP_VERSION) != null) {
			...
			for (@NotNull
			Entry<Object, Object> entry : newProperties.entrySet()) {
